...
interface ILogo {
  path: string;
  color: "black" | "primary" | "secondary" | "success" | "warning" | "danger";
  width: number;
  height: number;
}

export const Logo = ({ path, color, width, height }: ILogo) => {
  return (
    <Link to={path} className={styles.logo}>
      <svg
        className={cn(styles.logo__icon, styles[`logo__icon_color_${color}`])}
        stroke="currentColor"
        fill="currentColor"
        strokeWidth="0"
        viewBox="0 0 24 24"
